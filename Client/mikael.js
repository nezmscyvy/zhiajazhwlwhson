...
module.exports = toxic = async (client, m, chatUpdate, store) => {
    try {
        ...
        if (!settings) {
            console.error("MIKAEL-XD: Settings not found, cannot proceed!");
            return;
        }
        ...
        const filePath = require('path').resolve(__dirname, '../mikael.jpg');
        ...
        try {
            ...
            m.metadata = m.isGroup ? await client.groupMetadata(m.chat).catch(e => {
                console.error("MIKAEL-xD: Group metadata fetch error:", e);
                return {};
            }) : {};
            ...
            console.error("MIKAEL-XD: Error fetching group metadata:", error);
            m.metadata = {};
            m.isAdmin = false;
            m.isBotAdmin = false;
        }
        ...
        console.error('MIKAEL-XD Error:', util.format(err));
    }
    ...
    process.on('uncaughtException', function (err) {
        ...
        console.error('MIKAEL-XD Caught exception:', err);
    });
};