...
module.exports = {
  ...
  description: 'Displays the MIKAEL-XD command menu with interactive buttons',
  run: async (context) => {
    ...
    if (text) {
      ...
      console.log(`MIKAEL-XD: Rejected menu with text: ${text}`);
      return;
    }

    const settings = await getSettings();
    const effectivePrefix = settings.prefix || '.'; // Dynamic prefix from database
    console.log(`MIKAEL-XD: Generating menu with prefix: ${effectivePrefix}`);
    ...
    // Menu text with MIKAEL-XD flair
            ...
            buttons: [
              {
                ...
                buttonParamsJson: JSON.stringify({
                  ...
                  url: 'https://github.com/musicopilotvf456-eng/mikael-xd-',
                  merchant_url: 'https://github.com/musicopilotvf456-eng/mikael-xd-',
                }),
              },
              {
                name: 'single_select',
    ...
    console.log(`MIKAEL-XD: Sent interactive menu to ${m.chat} with prefix ${effectivePrefix}`);

    // Audio message logic
    const possibleAudioPaths = [
      path.join(__dirname, 'mikael_tech', 'menu.mp3'),
      path.join(process.cwd(), 'mikael_tech', 'menu.mp3'),
      path.join(__dirname, '..', 'mikael_tech', 'menu.mp3'),
    ];
    ...
    if (audioPath) {
      ...
      console.log(`MIKAEL-XD: Sent menu audio to ${m.chat}`);
    }
  },
};
